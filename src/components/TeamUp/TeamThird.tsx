...
function TeamThird() {
    ...
    const [hoverHover, setHoverHover] = useState<number | null>(null);
        ...
        <div>
            <div className="contentPadding py-10 grid grid-cols-1 gap-20">
                ...
                <div data-aos="fade-up" data-aos-delay={200} data-aos-once="true">
                    {jobData.map(items => {
                        const isExpanded = open === items.id;
                        const isHovered = hoverHover === items.id;
                        const isActiveStyle = isExpanded || isHovered;

                        return (
                            <div
                                key={items.id}
                                onMouseEnter={() => setHoverHover(items.id)}
                                onMouseLeave={() => setHoverHover(null)}
                                className={`transition-all duration-300 border rounded-xl mb-5 ${isActiveStyle ? 'border-transparent' : 'border-slate-200'}`}
                            >
                                <div
                                    onClick={() => setOpen(prev => (prev === items.id ? null : items.id))}
                                    className={`relative overflow-hidden grid grid-cols-3 lg:grid-cols-11 rounded-2xl py-5 px-5 sm:px-10 cursor-pointer transition-all duration-700 ease-in-out  ${
                                        isActiveStyle ? 'items-start bg-secondary  py-10 lg:py-20 ' : 'py-10 lg:py-20'
                                    }`}
                                >
                                    ...
                                    <div className="col-span-2 grid grid-cols-1 lg:col-span-9 lg:grid lg:grid-cols-9 lg:w-full gap-5 sm:gap-10 ">
                                        <p className={`heading-2 flex gap-5 sm:title-2-medium lg:w-full   lg:col-span-4 transition-colors duration-700 ${isActiveStyle ? 'text-primary' : ''}`}>
                                            {items.position} {isExpanded && <span className="hidden sm:block lg:hidden">→</span>}
                                        </p>
                                        <div className="sm:hidden">{isExpanded && <p className="text-slate-200 text-xs">{items.shortDescription}</p>}</div>
                                        <div className={` ${isExpanded ? 'lg:hidden' : 'hidden'} hidden lg:block lg:col-span-5 lg:col-start-7 text-justify`}>
                                            <p className="text-slate-400 text-base">{items.description}</p>
                                        </div>
                                        <div className="hidden sm:block lg:hidden ">{isExpanded && <p className="text-slate-400 text-xs">{items.description}</p>}</div>
                                        <img src={isActiveStyle ? gridLime : gridGray} className="absolute right-0  w-1/2  translate-y-1/5 -z-0 transition-opacity duration-700" />
                                    </div>
                                            ...
                                            {isExpanded && (
                                                <div className="absolute  -translate-x-1/3 w-[70%] gap-10">
                                                    <Swiper spaceBetween={10} slidesPerView={2} className="" grabCursor={true}>
                                                        {[img1, img2, img3].map((imgItem, idx) => (
                                                            <SwiperSlide key={idx} className=" relative overflow-hidden py-22 px-30 rounded-xl">
                                                                <img src={imgItem} alt="" className="absolute object-cover w-full h-full top-0 left-0" />
                                                            </SwiperSlide>
                                                        ))}
                                                    </Swiper>
                                                </div>
