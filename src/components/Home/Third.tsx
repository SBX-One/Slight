import img1 from '../../assets/imgs/Rectangle 483.webp';
import AOS from 'aos';
                    ...
                    <div className="">
                        {investingSteps.map(items => (
                            <div key={items.id} data-aos="fade-up" data-aos-delay={items.id * 100 + 150} data-aos-duration="1000">
                                ...
                                >
                                    <p
                                        className={`sm:heading-2 lg:title-2-medium text-secondary/20 transition-colors duration-300 group-hover:text-black/40 ${active === items.id ? `!text-white/50` : ''}`}
                                    >
                                        0{items.id}
                                    </p>
                                    <p
                                        className={`lg:hidden sm:heading-2 lg:title-2-medium text-black transition-transform duration-500 group-hover:translate-x-2 ${active === items.id ? `text-primary` : ''}`}
                                    >
                                        {items.title}
                                    </p>
                                    <div className="hidden lg:flex justify-between w-full">
                                        <p
                                            className={`sm:heading-2 lg:title-2-medium text-black transition-transform duration-500 group-hover:translate-x-4 ${active === items.id ? `text-primary` : ''}`}
                                        >
                                            {items.title}
                                        </p>
                                        <p
                                            className={`sm:heading-2 lg:title-2-medium text-black transition-transform duration-500 ease-in-out flex items-center justify-center group-hover:scale-125 ${active === items.id ? `text-primary rotate-90` : ``}`}
                                        >
                                            →
                                        </p>
                                    </div>
