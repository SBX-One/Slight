import cargoVideo from '../../assets/Videos/cargo.mp4';
import Herocard from './Herocard';
import AOS from 'aos';
import 'aos/dist/aos.css';
import { useEffect } from 'react';

function Hero() {
    useEffect(() => {
        AOS.init({
            duration: 800,
            once: true,
        });
    }, []);

    return (
        <div>
            <div className=" relative overflow-hidden">
                <div className="relative w-screen overflow-hidden lg:min-h-svh pt-[186px] ">
                    <video className="absolute top-0 left-0 w-full h-full object-cover -z-10" src={cargoVideo} autoPlay muted loop playsInline />

                    <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r lg:to-white/30 from-white  to-white/90 z-0" />
                    <div className="absolute hidden lg:block inset-0 bg-gradient-to-t lg:bg-gradient-to-r lg:to-white/30 from-white  to-white/90 z-0" />
                    <div className="relative z-10">
                        <div className="contentPadding py-[50px]  text-center">
                            <div className="lg:grid lg:grid-cols-8 items-center">
                                <div className="lg:col-span-4 " data-aos="fade-up">
                                    <div className="text-center flex flex-col gap-10 lg:items-start lg:text-start">
                                        <p className="title-2 sm:title-1 ">
                                            Transforming Maritime <br /> Shipping Investments{' '}
                                        </p>
                                        <p className="paragraph-2 sm:text-base">
                                            Slight is a platform that transforms investment in marine shipping by tokenizing them into smaller, affordable fractions
                                        </p>

                                        <div className="flex flex-col gap-5 items-center button-xs">
                                            <button className="primaryButton w-fit">
                                                <div className="flex gap-3 ">
                                                    <p>Mail Subscription</p>
                                                    <p>→</p>
                                                </div>
                                            </button>
                                            <button className="text-secondary/40 lg:hidden">Learn more</button>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 lg:flex py-20">
                                        <div className="flex gap-5  justify-end pr-5 items-center">
                                            <p className="heading-2 sm:text-4xl">
                                                100 <span className="text-primary">+</span>
                                            </p>
                                            <p className="sub-2 text-secondary/20">
                                                Companies <br /> Choosed
                                            </p>
                                        </div>
                                        <div className="flex gap-5 justify-start pl-5 items-center border-l border-secondary/10">
                                            <p className="heading-2 sm:text-4xl">
                                                44 <span className="text-primary"> +</span>
                                            </p>
                                            <p className="sub-2 text-secondary/20">Countries</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="lg:col-start-6 col-span-3 " data-aos="fade-up" data-aos-delay="400">
                                    <Herocard />
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default Hero;
